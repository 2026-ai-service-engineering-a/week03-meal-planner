...
| | v1.5 | v2.0 | v2.1 |
| --- | --- | --- | --- |
| 프롬프트 후보 | 210건 · 27,250자 | 46건 · 5,852자 | 46건 · 3,298자 |
| 계획자 입력 토큰 | 21,888 | 7,234 | 3,708 |
| 계획자 출력 토큰 | ~8,000 | ~8,000 | **2,902** |
| 전체 소요 (3회 평균) | | 73.4초 | **17.6초** |

v2.1이 한 일은 **산수를 코드로 되돌린 것**이다. 후보 줄의 숫자를 1인분 기준으로
미리 환산해 주고, 모델은 요일·식품코드·이유만 낸다. 나머지는 코드가 채운다 —
검증자와 `audit_plan()`이 어차피 같은 산수를 다시 하고 있었다.

**RAG가 값을 했는지는 따로 봐야 한다.** 벡터 파일을 치우고 같은 요청을 돌리면
(분류별 샘플로 강등) **같은 일곱 종이 나온다.** 500종 중 제약을 다 통과하는 것이
10개뿐이고 그 10개는 무조건 되살리기 때문이다. 검색이 실제로 필요해지는 규모는
원본 음식 DB(19,422종, 조건 통과 8,031건 = 279,248 토큰)부터다.
